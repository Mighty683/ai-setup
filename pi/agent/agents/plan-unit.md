...
Describe ordered waves and the reason assignments can or cannot run in parallel. Identify read-only research/review fanout separately from implementation. Independent mutation lanes may run concurrently only in separate managed Git worktrees with explicit, non-overlapping ownership and a clean committed baseline. Keep one writer per cwd/worktree; serialize shared-checkout, overlapping, and dependent changes. Define sequential integration, review, and final validation after each parallel wave, and serialize changes to shared contracts and the task file.
