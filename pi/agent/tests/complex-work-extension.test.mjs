...
function harness(t, execute) {
  ...
  let entries = [
    {
      type: "message",
      message: { role: "user", content: "Investigate the cache" },
    },
  ];
  let leaf = "user-leaf";
  const pi = {
  ...
  const bridge = rpc.registerSubagentRpcBridge({
    ...
    execute: async (_id, params) => {
      ...
      assert.ok(
        messages[0].content.includes("Main agent:"),
        "handoff guidance precedes launch",
      );
      return execute
        ? execute(params)
        : {
            content: [{ type: "text", text: "Started" }],
            details: { runId: `run-${launches.length}` },
          };
    },
  });
  const notifier = notify.default(
    pi,
    { currentSessionId: "parent-session", completionOwnerId: "owner" },
  ...
  return {
    ...
    taskPath: () =>
      entries.findLast((entry) => entry.customType === "complex-work-task")
        ?.data.taskPath,
  };
}

test("canonical commands and aliases use native async RPC with fresh context and the current checkout", async (t) => {
  const h = harness(t);
  assert.deepEqual(
    [...h.commands.keys()],
    ["research", "plan", "sergeant", "complex-work", "complex-work-plan"],
  );
  for (const [command, agent] of [
    ["research", "research-unit"],
    ["plan", "plan-unit"],
    ["sergeant", "sergeant-unit"],
    ["complex-work", "research-unit"],
    ["complex-work-plan", "plan-unit"],
  ]) {
    ...
    assert.equal(
      launch.isolation,
      undefined,
      "native RPC drops the legacy isolation hint; worktree:false is authoritative",
    );
  ...
  assert.match(
    h.launches[2].task,
    /user order authorizes execution; no separate plan approval/,
  );
  assert.match(
    h.launches[2].task,
    /no required descendant remains queued, running, or detached/,
  );
...
test("orders can start directly and repeat with feedback on the same task, without stage gates", async (t) => {
  const h = harness(t);
  await h.run("sergeant", "Execute the cache task");
  ...
  assert.match(h.launches[0].task, /REQUEST: Execute the cache task/);
  assert.match(
    h.launches[0].task,
    /Fresh session: no parent conversation is inherited/,
  );
...
test("quoted task paths persist across commands and extension reload, restore per session and reset cleanly", async (t) => {
  ...
  await h.run(
    "research",
    '--task "docs/tasks/cache work.md" Investigate caching',
  );
...
for (const missing of ["file", "leaf", "disk-file"]) {
  test(`missing parent ${missing} selects fresh before any entries or handoff are added`, async (t) => {
    ...
    assert.doesNotMatch(
      h.launches[0].task,
      /Use the forked conversation|current conversation/,
    );
  });
}
...
test("no objective, history, or existing task asks instead of launching, including metadata-only leaves", async (t) => {
  ...
  h.sessionStart(
    [
      {
        type: "custom",
        customType: "complex-work-task",
        data: { taskPath: path.join(h.ctx.cwd, "not-created.md") },
      },
    ],
    "selection-leaf",
  );
  ...
  for (const notice of h.notices)
    assert.match(notice.message, /Provide an objective/);
});

test("conversation history is never forked into unit launches", async (t) => {
  const h = harness(t);
  for (const entry of [
    {
      type: "message",
      message: { role: "user", content: [{ type: "text", text: "Fix cache" }] },
    },
    ...
    await h.run("plan", "Plan the cache fix");
    assert.equal(h.launches.at(-1).context, "fresh");
    assert.match(
      h.launches.at(-1).task,
      /Fresh session: no parent conversation is inherited/,
    );
  }
...
test("native completion returns all unit results without launching another stage", async (t) => {
  ...
  for (const command of ["research", "plan", "sergeant"]) {
    ...
    assert.equal(
      h.messages.length,
      before,
      "runtime deduplicates completion delivery",
    );
  }
});

test("native launch failures are reported and never retried", async (t) => {
  const h = harness(t, async () => ({
    isError: true,
    content: [{ type: "text", text: "Native launch failed" }],
    details: {},
  }));
  await h.run("plan", "Plan the cache fix");
  assert.equal(h.launches.length, 1);
  assert.equal(h.launches[0].context, "fresh");
  assert.equal(h.notices.at(-1).level, "error");
  assert.match(h.messages.at(-1).content, /Native launch failed/);
  assert.equal(h.messages.at(-1).options.triggerTurn, false);
});
...
test("acknowledgement timeout leaves no listeners or duplicate launches", async () => {
  ...
  await assert.rejects(
    spawnSubagent({ events }, { agent: "research-unit" }, 5),
    /may already be running/,
  );
  assert.equal(requests.length, 1);
  assert.deepEqual(events.bus.eventNames(), ["subagents:rpc:v1:request"]);
  events.emit(`subagents:rpc:v1:reply:${requests[0].requestId}`, {
    success: true,
    data: { details: { runId: "late" } },
  });
  assert.equal(requests.length, 1);
});
...
test("concurrent launch acknowledgements stay correlated", async () => {
  ...
  for (const request of [...requests].reverse())
    events.emit(`subagents:rpc:v1:reply:${request.requestId}`, {
      success: true,
      data: { details: { runId: request.params.agent } },
    });
...
test("installed agent discovery resolves standalone artifact-writing profiles without read-only ceilings", (t) => {
  ...
  cpSync(path.join(agentRoot, "agents"), path.join(cwd, ".pi/agents"), {
    recursive: true,
  });
  const settings = JSON.parse(
    readFileSync(path.join(agentRoot, "settings.json"), "utf8"),
  );
  writeFileSync(
    path.join(cwd, ".pi/settings.json"),
    JSON.stringify({ subagents: settings.subagents }),
  );
  ...
  assert.equal(
    existsSync(path.join(agentRoot, "lib/complex-work/read-only.ts")),
    false,
  );
  for (const name of [
    "research-unit",
    ...
    for (const tool of ["subagent", "edit", "write"])
    ...
    assert.match(
      agent.systemPrompt,
      /unused collision-safe name under docs\/tasks\//,
    );
    ...
    assert.match(
      agent.systemPrompt,
      /never blindly overwrite the whole dossier/,
    );
    ...
    if (name === "sergeant-unit") {
      for (const term of [
        ...
        "completion comments",
        "acceptance evidence",
        "Stop on infrastructure errors",
        "no plan exists",
        "do not demand a separate plan approval",
        "runs.all",
        "bg_wait",
        "required descendant",
