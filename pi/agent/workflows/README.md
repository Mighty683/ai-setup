...
5. Launch `complex-work-review-wave.js`. Reviewers return structured `pass` or `block` verdicts. A fully passing review returns `review-passed`. Any blocker returns `review-decision-required`; present the findings to the user and wait. If the user chooses `replan`, call `complex_work_control` with action `replan` and rerun planning on the same mission. If that planning run fails validation, record it with `plan-failed` and result status `planning-failed`, then use `retry-plan`; the rejected review remains preserved until a valid replacement addresses every finding. The revised plan preserves completed wave IDs and includes the rejected wave and findings, forming an explicit review → user decision → plan → execute loop. The user may instead abandon the mission.
6. Only after `review-passed`, obtain explicit user verification of the runnable wave, then launch `complex-work-verify-wave.js` and `complex-work-close-wave.js` in that order. Repeat from step 3 for the next wave.
...
Workflow scripts enforce sequencing and isolation. Lane, integration, and review workflows use structured semantic outcomes; a child process completing is not itself evidence that its work passed. They cannot resolve semantic conflicts or obtain user authority. When review returns `review-decision-required`, the root coordinator must present the findings and wait for the user to choose `replan` or `abandon`. The `complex-work` extension exposes the valid next transition as an opt-in tool and blocks direct out-of-order complex-work workflow launches while a session is active. Advance a phase with `complex_work_control complete` only by passing the exact workflow `resultStatus` requested by the control tool; failed or blocked execution and integration workflows leave the current phase unchanged.
