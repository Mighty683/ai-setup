...
Object.entries(envApiKeys).forEach(([provider, key]) => {
	if (key) {
...
Feel free to use these tools when needed to provide accurate and helpful responses.`,
	...
	await chatPanel.setAgent(agent, {
		onApiKeyRequired: async (provider: string) => {
			...
			return envApiKeys[provider as keyof typeof envApiKeys] || "";
		},
			...
			<div class="flex items-center justify-between border-b border-border shrink-0">
				<div class="flex items-center gap-2 px-4 py-2">
					...
					${
						currentTitle
							...
							: html`<span class="text-base font-semibold text-foreground">Pi Web UI App</span>`
					}
				</div>
				<div class="flex items-center gap-1 px-2">
					...
					${Button({
						...
						onClick: () => SettingsDialog.open([new ProxyTab()]),
						title: "Settings",
					})}
				</div>
			</div>
